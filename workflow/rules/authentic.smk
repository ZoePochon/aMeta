...
rule Authentication_Plots:
    ...
    shell:
        ...
        if [[ -n "$CONDA_DEFAULT_ENV" ]]; then
            ...
            $env/bin/Rscript {params.exe} {wildcards.taxid} {wildcards.sample}.trimmed.rma6 {input.dir}/ &> {log}
        else
            # Environment Modules: Assume modules are loaded
            echo "Using environment modules for R..."
            Rscript {params.exe} {wildcards.taxid} {wildcards.sample}.trimmed.rma6 {input.dir}/ &> {log}
        fi
    """
